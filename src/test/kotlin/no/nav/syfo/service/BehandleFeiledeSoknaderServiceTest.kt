...
class BehandleFeiledeSoknaderServiceTest {
    ...
    fun setup() {
        ...
        given(saksbehandlingsService.opprettJournalpost(anyString(), any(), anyString())).willReturn("journalpostId")
        given(aktorConsumer.finnFnr("aktorId")).willReturn("fnr")
        given(saksbehandlingsService.opprettSoknad(any(), anyString())).willReturn(Soknad.lagSoknad(sykepengesoknad, "fnr", "Ola Nordmann"))
    }
    ...
    fun rebehandlerInnsendingSomHarFeiletIAktor() {
        ...
        verify<SaksbehandlingsService>(saksbehandlingsService).opprettOppgave(eq("innsendingsId"), anyString(), any(), any(), anyString(), anyString())
    ...
    fun rebehandlerInnsendingSomHarFeiletISak() {
        ...
        verify<SaksbehandlingsService>(saksbehandlingsService).finnEllerOpprettSak(anyString(), anyString(), any())
        verify<SaksbehandlingsService>(saksbehandlingsService).opprettJournalpost(anyString(), any(), anyString())
        verify<SaksbehandlingsService>(saksbehandlingsService).opprettOppgave(anyString(), anyString(), any(), any(), anyString(), anyString())
