...
fun skapJournalpostRequest(
    ...
    return JournalpostRequest(
        bruker = Bruker(id = soknad.fnr!!, idType = "FNR"),
        dokumenter =
            listOf(
                Dokument(
                    dokumentvarianter =
        ...
        kanal = "NAV_NO",
        tittel = getJornalfoertDokumentTittel(soknad = soknad),
        avsenderMottaker = AvsenderMottaker(id = soknad.fnr, idType = "FNR"),
    )
}
...
private fun getJornalfoertDokumentTittel(soknad: Soknad): String {
    return when (soknad.soknadstype) {
        ...
        Soknadstype.SELVSTENDIGE_OG_FRILANSERE ->
            "Søknad om sykepenger fra Selvstendig/Frilanser for periode: ${
                soknad.fom!!.format(
                    DatoUtil.norskDato,
                )
            } til ${soknad.tom!!.format(DatoUtil.norskDato)}"

        Soknadstype.ARBEIDSTAKERE -> "Søknad om sykepenger ${soknad.fom!!.format(DatoUtil.norskDato)} - ${
            soknad.tom!!.format(
                DatoUtil.norskDato,
            )
        }"
        ...
        Soknadstype.ARBEIDSLEDIG ->
            "Søknad om sykepenger fra arbeidsledig for periode: ${
                soknad.fom!!.format(
                    DatoUtil.norskDato,
                )
            } til ${
        ...
        Soknadstype.ANNET_ARBEIDSFORHOLD ->
            "Søknad om sykepenger med uavklart arbeidssituasjon fra ${
                soknad.fom!!.format(
                    DatoUtil.norskDato,
                )
            } til ${soknad.tom!!.format(DatoUtil.norskDato)}"
        ...
        Soknadstype.REISETILSKUDD ->
            "Søknad om reisetilskudd for periode: ${
                soknad.fom!!.format(
                    DatoUtil.norskDato,
                )
            } til ${
        ...
        Soknadstype.GRADERT_REISETILSKUDD ->
            "Søknad om sykepenger med reisetilskudd for periode: ${
                soknad.fom!!.format(
                    DatoUtil.norskDato,
                )
            } til ${soknad.tom!!.format(DatoUtil.norskDato)}"
